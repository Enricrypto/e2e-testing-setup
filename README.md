...
├── global-setup.ts               # Pre-test setup (seed data)
├── global-teardown.ts            # Post-test cleanup
│
├── tests/
│   ├── fixtures.ts               # Auth fixtures, test setup
│   ├── 01-example/
│   │   ├── example.spec.ts       # Example test (reference)
│   │   └── README.md
│   └── 02-your-feature/          # Your feature tests
│       ├── feature.spec.ts       # Generated by pipeline
│       └── README.md
│
├── pom/
│   ├── BasePage.ts               # Base class (semantic locators)
│   ├── ExamplePage.ts            # Example POM class
│   └── YourFeaturePage.ts        # Generated by pipeline
│
└── utils/
...
docs/
├── E2E_DEEP_AUDIT_CHECKLIST.md   # Phase 0 audit (read first)
├── E2E_PIPELINE_AUDIT.md         # Phase 8 audit (read after tests)
├── PHASE_3_AUTOMATED_PIPELINE.md # Pipeline flow (reference)
└── E2E_SEMANTIC_LOCATORS.md      # Locator patterns (reference)

scripts/
└── phase3-pipeline.sh            # The orchestrator (runs all 8 steps)
```
